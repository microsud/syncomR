#' @title Plot stability properties Resistance (simplified)
#'
...
#' plot_sref_dev(dat.stab, method = "canberra", palette = "brown3")
#' @author Contact: Sudarshan A. Shetty \email{sudarshanshetty9@gmail.com}
#'
#' @export
#' @keywords Anlaysis and visualization
#'
plot_sref <- function(stab.in,
                      method = c("canberra", "euclidean"),
                      palette = "grey70") {
  ...
  if (method == "euclidean") {
    df.1 <- subset(df.1, DistanceMethod == "euclidean")
    lab.txt <- paste(
      " Euclidean, RS=", round(RSeuclidean, digits = 4), "| DS=", round(DisSpeedeuclidean, digits = 4)
    )
  } else if (method == "canberra") {
    df.1 <- subset(df.1, DistanceMethod == "canberra")
    lab.txt <- paste(
      "Canberra, RS=", round(RScanberra, digits = 4), "| DS=", round(DisSpeedcanberra, digits = 4)
    )
  } else {
    stop(message("medthod must be either euclidean or canberra"))
  }
  p <- ggplot(df.1, aes(Time, Distance)) +
    geom_line(color = palette, size = 0.6, alpha = 0.8) +
    ylim(c(0, 1)) +
    geom_point(aes(shape = Variable), color = palette, size = 2)
  ...
  if (method == "euclidean") {
    p <- p + geom_hline(aes(yintercept = r_Euclidean),
      colour = palette, linetype = "dashed"
    ) + ylab(expression(paste("Community evolve apart ", s[ref]))) + ggtitle(label = "", subtitle = lab.txt)
  } else if (method == "canberra") {
    p <- p + geom_hline(aes(yintercept = r_Canberra),
      colour = palette, linetype = "dashed"
    ) + ylab(expression(paste("Community evolve apart ", s[ref]))) + ggtitle(label = "", subtitle = lab.txt)
  }
  return(p)
