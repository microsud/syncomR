...
plot_resilience_euclidean <- function(stab.in){
  ...
  p <- ggplot(df.2, aes(Time, RLeuclidean, label=Time)) + theme_syncom() +
    geom_line(size = 0.6, alpha = 0.8,color = "deepskyblue3") + ylim(c(0, 1)) +
    geom_point(aes(shape = Variable), color = "deepskyblue3",size = 2) + scale_color_manual(values = c(
  ...
  p <- p + scale_shape_manual(values = c(
    ...
    No = 16,
    ExperimentEnd = 0
  )) + ylab(expression(~Resilience[Euclidean])) #+ geom_label_repel(subset(df.2, Time== df.2[1,1]), aes(Time,RLeuclidean))

  lab.txt <-  paste("RL=", round(data$RLeuclidean[nrow(data)], digits = 4),
...
plot_resilience_canberra <- function(stab.in){
  ...
  p <- ggplot(df.2, aes(Time, RLcanberra, label=Time)) + theme_syncom() +
    geom_line(size = 0.6, alpha = 0.8,color = "brown3") + ylim(c(0, 1)) +
    geom_point(aes(shape = Variable), color = "brown3",size = 2) + scale_color_manual(values = c(
      RLeuclidean = "brown3")) +
    geom_hline(yintercept = 0,
               colour = "black", linetype = "dashed"
    )
  p <- p + scale_shape_manual(values = c(
    ...
    No = 16,
    ExperimentEnd = 0
  )) + ylab(expression(~Resilience[Canberra]))
  lab.txt <-  paste("RL=", round(data$RLcanberra[nrow(data)], digits = 4),
