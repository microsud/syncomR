...
plot_stability_properties <- function(stab.in, color.euc = "steelblue",
                                      color.can = "brown3",
                                      text.size = 1.2,
                                      property = c("dist.ot", "resilience.ot.eucl",
  ...
  if(property == "dist.ot"){
    ...
    p <- ggplot(plotData) + theme_syncom()
    # euclidean
    p <- p + geom_point(aes(Time,euclidean), color=color.euc) +
      geom_line(aes(Time,euclidean), color=color.euc) + geom_hline(aes(yintercept=r_Euclidean), color=color.euc, linetype="dashed")
    #canberra
    p <- p + geom_point(aes(Time,canberra), color=color.can) +
      geom_line(aes(Time,canberra), color=color.can) + geom_hline(aes(yintercept=r_Canberra), color=color.can, linetype="dashed") +
      scale_y_continuous(limits = c(0, 1))
    #endpoints
    df.hold1 <- subset(plotData, Time == experimentEnd)
    p <- p + geom_point(data=df.hold1, aes(Time,euclidean), fill="black") +
      geom_point(data=df.hold1, aes(Time,canberra), fill="black")
    #timelabels
    p < p + geom_text_repel(data=plotData, aes(Time,euclidean, label=Time), size=2) +
      geom_text_repel(data=plotData, aes(Time,canberra, label=Time), size=2)

    #max deviations

    p <- p + geom_point(data=subset(data, Time == data[maxEuclideanId, 1]),
                   aes(Time,euclidean),
                   color=color.euc, shape= 24) +
      geom_point(data=subset(data, Time == data[maxCanberraId, 1]),
                 aes(Time,canberra),
                 color=color.can, shape= 24)
    euc.tax <- paste0("Euclidean, RS=", toString(round(RSeuclidean, digits=4)), ", DS=", toString(round(DisSpeedeuclidean, digits=4)))
    Euclidean.RS <- paste("Euclidean-RS=", round(RSeuclidean, digits=4))


    p <- p + annotate("label", x = data$Time[length(data$Time)-3], y = 0.9,
                  label = Euclidean.RS)
    p + labs(title = "Resistance and displacement speed\n computing",
                                                    subtitle = paste0("Euclidean, RS=", toString(round(RSeuclidean, digits=4)),
                                                                      ", DS=", toString(round(DisSpeedeuclidean, digits=4)),
                                                                      "Canberra, RS=", toString(round(RScanberra, digits=4)),
                                                                      ", DS=", toString(round(DisSpeedcanberra, digits=4))) +
      ylab(expression('Deviation (d) from s'[ref])) + xlab("Time")
