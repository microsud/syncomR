...
#'
#' @description Online resilience of the community as decribed by Florian Centler and Zishu Liu, UFZ Leipzig, Germany.
#'
...
#'  This functions takes output from the \code{\link{stability_properties}}. The closer it is to zero the system is
#'  deviating continously away from reference state, while values closer to 1 indicate the onset of recovery
#'  (1 = perfect recovery).
...
plot_resilience_online <- function(stab.in,
  ...
  p <- p + geom_hline(yintercept = 0, linetype = "dashed") + ylab(expression(~ Resilience[Online - Canberra]))
  return(p)
}
