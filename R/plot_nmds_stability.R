...
#'
#' @description NMDS plot compared to reference states as described by
...
#'
#' @param max.distant.states.euc Color for sites or timepoint that are max distant,
#'                               Euclidean DEFAULT = steelblue
#'
#' @param max.distant.states.can Color for sites or timepoint that are max distant,
#'                              Canberra. DEFAULT = brown3
#'
...
#' \item{}{max.distant.states.euc= Color for sites/timepoint that are max distant,
#'         Euclidean DEFAULT = steelblue}
#' \item{}{max.distant.states.can = Color for sites/timepoint that are max distant,
#'         Canberra. DEFAULT = brown3}
...
plot_nmds_stability <- function(stab.in,
  ...
  mds.out <- metaMDS(rbind(data[, 2:(numberOfGates + 1)], referenceState),
    distance = "bray",
    autotransform = FALSE,
    zerodist = "add"
  )
  ...
  # refphase
  p <- p + geom_point(
    data = nmd.dat[c(1:trefId), ], aes(MDS1, MDS2),
    fill = refphase, size = 4, shape = 21
  )
  ...
  p <- p + labs(
    title = "",
    subtitle = expression(paste("Community evolve apart ", s[ref]))
  )
