...
#'
#' @details Calculates and plots coverage of species from a phyloseq object and returns a ggplot object. Useful for checking co-existence over
...
#' accounting for 95 percent relative abundance in each sample timepoint is ploted
...
#' @param color.variable Name of column in sample_data specifying variable to use for coloring
...
taxa_coverage <- function(ps,
  ...
  if (plot == TRUE) {
    p <- ggplot(data.frame(diversity, sample_data(ps))) +
      geom_point(
        aes_string(
          x = time.column,
